        ...
        <aside className="w-full lg:w-64 flex-shrink-0">
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
                ...
                <nav className="p-2 space-y-1">
                    ...
                    <Link href="/dashboard/verify-payments" className={`flex items-center justify-between px-4 py-3 font-medium rounded-xl transition-colors ${activeTab === 'verify-payments' ? 'bg-purple-50 text-kh-purple' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
                        <div className="flex items-center gap-3">
                            <BadgeCheck className="h-5 w-5" /> Verify Payments
                        </div>
                        {(user.pendingDeliveryCount > 0) && (
                            <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{user.pendingDeliveryCount}</span>
                        )}
                    </Link>
