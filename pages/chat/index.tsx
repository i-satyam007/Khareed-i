...
export default function ChatPage() {
    ...
    return (
        <div className="min-h-screen bg-kh-light font-sans flex flex-col">
            <Head>
            ...
            <div className="flex-1 container mx-auto px-4 py-6 flex gap-6 h-[calc(100vh-100px)]">
                ...
                <div className={`w-full md:w-80 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-sm overflow-hidden flex flex-col ${selectedUser ? 'hidden md:flex' : 'flex'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {conversations?.map((conv: any) => (
                            <div
                                ...
                                className={`p-4 flex items-center gap-3 cursor-pointer hover:bg-white/5 transition-colors ${selectedUser?.id === conv.otherUser.id ? 'bg-purple-500/10' : ''}`}
                            >
                                <div className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center overflow-hidden shrink-0">
                                    {conv.otherUser.avatar ? <img src={conv.otherUser.avatar} className="w-full h-full object-cover" /> : <User className="h-5 w-5 text-gray-400" />}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <h3 className="font-bold text-white text-sm truncate">{conv.otherUser.name}</h3>
                ...
                {/* Chat Area */}
                <div className={`flex-1 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 shadow-sm overflow-hidden flex flex-col ${!selectedUser ? 'hidden md:flex' : 'flex'}`}>
                    {selectedUser ? (
                        <>
                            {/* Chat Header */}
                            <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5 z-10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="w-10 h-10 bg-white/10 rounded-full flex items-center justify-center overflow-hidden">
                                        {selectedUser.avatar ? <img src={selectedUser.avatar} className="w-full h-full object-cover" /> : <User className="h-5 w-5 text-gray-400" />}
                                    </div>
                                    <div>
                                ...
                                {messages?.map((msg: any) => {
                                    ...
                                    return (
                                        <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[70%] rounded-2xl px-4 py-2 text-sm ${isMe ? 'bg-kh-purple text-white rounded-br-none' : 'bg-white/10 border border-white/10 text-white rounded-bl-none shadow-sm'}`}>
                                                <p>{msg.content}</p>
                            ...
                            <form onSubmit={handleSendMessage} className="p-4 bg-white/5 border-t border-white/10 flex gap-2">
                                <input
                                    ...
                                    className="flex-1 bg-black/20 border border-white/10 rounded-xl px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-kh-purple/20 focus:border-kh-purple transition-all"
                                />
                                <button
                                    type="submit"
