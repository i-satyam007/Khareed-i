                ...
                {orders && orders.length > 0 ? (
                    <div className="space-y-6">
                        {orders.map((order: any) => (
                            <div key={order.id} className="bg-white/5 backdrop-blur-md rounded-xl border border-white/10 overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                                <div className="bg-white/5 px-6 py-4 border-b border-white/10 flex flex-wrap items-center justify-between gap-4">
                                    ...
                                    <div className="px-3 py-1 bg-green-500/10 text-green-400 border border-green-500/20 text-xs font-bold rounded-full uppercase tracking-wide">
                                        {order.status}
                                    </div>
                                </div>
                                    ...
                                    {order.items.map((item: any) => (
                                        <div key={item.id} className="flex gap-6">
                                            ...
                                            <div className="flex-1">
                                                <h3 className="text-lg font-bold text-white mb-1">{item.listing.title}</h3>
                                                <p className="text-sm text-gray-400 mb-4">Sold by <span className="font-medium">{item.listing.owner.name}</span></p>
                    ...
                    <div className="text-center py-16 bg-white/5 backdrop-blur-md rounded-xl border border-white/10 border-dashed">
